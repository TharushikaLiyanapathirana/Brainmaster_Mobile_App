...
export default function SpeedRelated(props){

    const [speedRelatedPapers, setSpeedRelatedPapers] = useState(null);
    const [isLoading, setIsLoading] = useState(true);

    const getSpeedRepatedPapers = async ()=>{
        ...
        await axios.post(config.API_URL+'/iq_2', requestBody)
          .then(function (response) {
            if(response.status===200){
                setSpeedRelatedPapers(response.data);
                setIsLoading(false);
            }
          })
    ...
    return(
        <View style={styles.container}>
            <ScrollView showsVerticalScrollIndicator={false}>
                {isLoading?false : speedRelatedPapers.map((item, index)=>{
                    return(
