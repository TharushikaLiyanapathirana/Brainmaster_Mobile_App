...
export default function LogicRelated(props){

    const [logicRelatedPapers, setLogicRelatedPapers] = useState(null);
    const [isLoading, setIsLoading] = useState(true);

    const getLogicRelatedPapers = async ()=>{
        ...
        await axios.post(config.API_URL+'/iq_2', requestBody)
          .then(function (response) {
            if(response.status===200){
                setLogicRelatedPapers(response.data);
                setIsLoading(false);
            }
          })
    ...
    return(
        <View style={styles.container}>
            <ScrollView showsVerticalScrollIndicator={false}>
                {isLoading?false : logicRelatedPapers.map((item, index)=>{
                    return(
