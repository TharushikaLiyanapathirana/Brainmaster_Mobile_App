...
export default function NumberSequence(props){

    const [numberSequencePapers, setNumberSequencePapers] = useState(null);
    const [isLoading, setIsLoading] = useState(true);

    const getNumberSequencePapers = async ()=>{
        ...
        await axios.post(config.API_URL+'/iq_2', requestBody)
          .then(function (response) {
            if(response.status===200){
                setNumberSequencePapers(response.data);
                setIsLoading(false);
            }
          })
    ...
    return(
        <View style={styles.container}>
            <ScrollView showsVerticalScrollIndicator={false}>
                {isLoading?false : numberSequencePapers.map((item, index)=>{
                    return(
