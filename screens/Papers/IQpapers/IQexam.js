...
import { Button, Divider , RadioButton, TextInput, Paragraph, Dialog, Portal } from 'react-native-paper';
...
export default function IQExam(props){
    ...
    const getPaper = async () =>{
        try{
            ...
            .then((res)=>{
                console.log("Selected Paper : "+res.data);
                setSelectedPaper(res.data);

                axios.get(config.API_URL+"/user/"+userId)
                .then((res)=>{
                    setParticipantUser(res.data);
    
                    let requestBody = {}
    
                    axios.post(config.API_URL+"/paper2/"+selectedPaperId, requestBody)
                    .then((res)=>{
                        let mcq =[];
                        let struct = [];
    ...
    const submitExam = async () =>{
        ...
        try{
            await AsyncStorage.setItem("iqExamMarking", JSON.stringify(markings));
        }catch(e){
            console.log(e.message);
        }
        ...
        .then((response) => {
            ...
            axios.post(config.API_URL+"/user/"+participantUser.user_id, newRequestBody).then((res)=>{
                props.navigation.navigate('IQ Result');
            })
        })
    ...
    return(
        ...
        {isLoading?false : <View>
            ...
            <View style={styles.detailsContainer}>
                {/* <Text style={styles.detailsHeadingText}> GK Exam : </Text> */}
                ...
                <ScrollView showsVerticalScrollIndicator={false}>
                    ...
                    {structuredQuestions.map((question, index)=>{
                        return(
                            <View key={question.question_id} style={styles.newsContainer}>
                                <View style={styles.newsWrapper}>
                                    <Text style={styles.newsTitle}>Question {index+6}</Text>
                                </View>
                                <Divider style={styles.divider1} />
                                ...
                                <View style={styles.newsDetailsWrapper}>
                                    ...
                                    <TextInput
                                        label="Enter short answer"
                                        value={getAnswer(index+5)}
                                        mode={'outlined'}
                                        onChangeText={(value)=>setAnswer(index+5, value)}
                                        style={styles.input1}
                                    />
                                </View>
                            </View>
