...
export default function Health(props){
    ...
    const [healthPapers, setHealthPapers] = useState(null);
    const [isLoading, setIsLoading] = useState(true);

    const getHealthPapers = async ()=>{
    ...
    const getTestId = async ()=>{
        await AsyncStorage.getItem('selected_paper_id').then((val)=>{
            console.log("Paper ID : "+val);
        })
    }
    ...
    return(
        <View style={styles.container}>
            <ScrollView showsVerticalScrollIndicator={false}>
                {isLoading?false:  healthPapers.map((item, index)=>{
                    return(
