...
export default function Politics(props){
    ...
    const [politicsPapers, setPoliticsPapers] = useState(null);
    const [isLoading, setIsLoading] = useState(true);

    const getPoliticsPapers = async ()=>{
        ...
        await axios.post(config.API_URL+'/iq_2', requestBody)
          .then(function (response) {
            if(response.status===200){
                setPoliticsPapers(response.data);
                setIsLoading(false);
            }
          })
    ...
    return(
        <View style={styles.container}>
            <ScrollView showsVerticalScrollIndicator={false}>
                {isLoading?false : politicsPapers.map((item, index)=>{
                    return(
