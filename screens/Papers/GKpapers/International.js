...
export default function International(props){
    ...
    const [internationalPapers, setInternationalPapers] = useState(null);
    const [isLoading, setIsLoading] = useState(true);

    const getInternationalPapers = async ()=>{
        ...
        await axios.post(config.API_URL+'/iq_2', requestBody)
          .then(function (response) {
            if(response.status===200){
                setInternationalPapers(response.data);
                setIsLoading(false);
            }
          })
    ...
    return(
        <View style={styles.container}>
            <ScrollView showsVerticalScrollIndicator={false}>
                {isLoading?false : internationalPapers.map((item, index)=>{
                    return(
